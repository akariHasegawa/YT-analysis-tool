...
- 【絶対禁止：セリフ・ナレーション指示】
  動画生成AIは映像のみ生成するため、セリフ・台詞・ナレーションは一切書かないこと
  ✗ 禁止：「〜と言いながら」「〜と叫ぶ」「〜と呟く」「ナレーションが入る」「〜と返す」
  ✓ 正しい変換：「大きく口を開け、驚愕の表情で後ずさりする」「両腕を広げて口をパクパクさせながらよろける」
- 【コメディ・リアクション系の場合】
  大げさなリアクション・スローモーション・コミカルな物理描写を具体的に書くこと
  例：「スローモーションで転倒し、周囲のアイテムが次々と倒れる」「目を見開き飛び上がって天井近くまで上がる誇張表現」

【出力形式】
各シーンは動画生成AIで1回生成できる5〜10秒の映像として設計すること。
...
🎬 シーン4｜締め（約5〜10秒）
[動画生成AIに直接貼れる日本語プロンプト。シーン3から自然につながる映像を100〜150字で記述]
...
const VIDEO_FROM_SCRIPT_PROMPT = `あなたは動画生成AIに渡すプロンプトの専門家です。
渡された台本をもとに、動画生成AIが正確に映像を生成できるシーン別プロンプトを作成してください。

【絶対禁止：セリフ・ナレーション指示を書かないこと】
動画生成AIは映像のみを生成します。セリフ・台詞・ナレーションは一切書かないこと。
✗ 悪い例：「Aが『やばい！』と叫ぶ」「ナレーションが入る」「〜と言いながら」「〜と呟く」「〜と返す」
✓ 良い例：「Aが両手を広げて口を大きく開け、驚愕の表情で後ずさりする」
セリフは必ず「表情・体の動き・リアクション」に変換すること。

【コメディ・リアクション映像の書き方】
台本がコメディ・ギャグ・ドタバタ系の場合、以下を必ず反映すること：
- 大げさなリアクション：「目を見開き口を大きく開けて飛び上がる」「両腕を大きく振り回しながらよろける」
- スローモーション演出：「スローモーションで転倒していく様子、周囲のアイテムが次々と倒れる」
- コミカルな物理描写：「ボールが的外れなコーンに直撃、ドミノ倒しで周囲が混乱する様子を横から映す」
- 対比・落差：「自信満々に構えるAをBが呆れた表情で横目で見ている、正面からのショット」
抽象的な「コメディタッチ」「面白い動き」は禁止。具体的な動作・表情・状況を書くこと。

【ビジュアルの具体化】
- 登場人物：衣装の色・形・小道具を必ず書く（例：「青いTシャツ・黒の短パン・サッカーシューズを履いた男性」）
- 背景・小道具：場所と周囲のアイテムを具体的に（例：「公園のサッカーコート、カラーコーンとゴールが背景に見える」）
- カメラ構図：必ずカメラ位置・向き・アングルをセットで書く
  例：「カメラは正面から」「横からのサイドショット」「俯瞰で見下ろす」「低アングルで見上げる」
...
🎬 シーン2｜本編A（約5〜10秒）
[映像のみの描写。100〜150字。シーン1から自然につながる映像]

🎬 シーン3｜本編B（約5〜10秒）
[映像のみの描写。100〜150字。シーン2から自然につながる映像]

🎬 シーン4｜締め（約5〜10秒）
[映像のみの描写。100〜150字。シーン3から自然につながる映像]

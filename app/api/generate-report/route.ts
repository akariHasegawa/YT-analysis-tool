...
## 必須構成（この順番で全セクションを必ず出力すること）
1. 挨拶（約1分）
2. 分析概要・視聴維持率スコア説明（約2分）
3. HOOK・EMOTION・CTA・STRUCTURE の詳細説明（約3分）
4. 改善提案（improvementIdeasの全項目を読み上げ台本として記載）（約3分）
5. 次回コンテンツ案（nextVideoIdeasの全項目を記載）（約2分）
6. まとめ・次回アクション（約1分）
7. 想定Q&A
...
- 各セクションに所要時間の目安を記載
- 重要ポイントには「★」マーク
- 改善提案・次回コンテンツ案は分析データの内容をそのまま使い、省略しない
- ライトテーマ（背景 #f8f9fa）
- 完全なHTMLを返す（<!DOCTYPE html>から</html>まで）
- マークダウンのコードブロック不要、HTMLを直接返す
- インラインCSSのみ使用
- A4縦向きに最適化`
}

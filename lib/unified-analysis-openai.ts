...
- nextVideoIdeas は必ずこの動画のジャンル・トーンを踏まえたアイデアにすること。コミカル・アクシデント系ならコミカルなアイデア、教育系なら教育系、感動系なら感動系。絶対に「自己成長チャレンジ」「モチベーション」などの汎用テーマにデフォルトしないこと。ハッシュタグ・BGM・タイトル・コメントからジャンルを推定して反映すること。
...
【ビジュアル系コンテンツ分析モード】
この動画は字幕・音声テキストが存在しないビジュアル系コンテンツです。
まずハッシュタグ・タイトル・BGM・コメントからジャンルを推定すること（コミカル・アクシデント系／ダンス系／筋トレ系／料理系／日常Vlog系など）。
推定したジャンルを analysis の hook/emotion/nextVideoIdeas に必ず反映すること。
以下の観点で分析してください：
